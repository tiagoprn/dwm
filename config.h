...
static Key keys[] = {
	...
	/* description: (cmd) increase sound volume */		{ Mod1Mask,			XK_Up,			spawn,          SHCMD("pactl set-sink-volume $(pacmd list-sink-inputs | grep -c index) +5% && kill -44 $(pidof dwmblocks)") },
	/* description: (cmd) decrease sound volume */		{ Mod1Mask,			XK_Down,		spawn,          SHCMD("pactl set-sink-volume $(pacmd list-sink-inputs | grep -c index) -5% && kill -44 $(pidof dwmblocks)") },
